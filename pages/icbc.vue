...
export default {
  ...
  async asyncData({ $axios }) {
    //批量获取数据
    const [seoData, data, imgData] = await Promise.all([$axios.$post('/admins/seoInfo', { code: 'icbc' }), $axios.$get('/api/105-32?bankCode=ICBC', { timeout: 3000 }), $axios.post('/admins/imgList')])
    //解析head数据
