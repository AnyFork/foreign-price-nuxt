<template>
  <div class="content">
    <!--顶部title -->
    <title-item title="今日招商银行外汇牌价汇率表" :subtitle="'招商银行外汇牌价更新时间：' + refresh"></title-item>
    <a-divider />
    <!--广告位 -->
    ...
    <a-divider />
    <!--文本摘要 -->
    <Summary title="招商银行外汇牌价表使用说明">
      <template slot="text">
        提供中国招商银行当日美元、欧元、日元等10种主要货币兑换人民币的即时外汇牌价表，单位为人民币兑100外币。表中的<strong>现汇买入价:</strong>指招商银行买入外汇的汇率，<strong>现钞买入价:</strong>指银行买入外币现钞的汇率，<strong>现汇卖出价:</strong>指银行卖出外汇的汇率，<strong>现钞卖出价:</strong>指银行卖出外钞的汇率。外汇牌价数据随招行营业网点实时更新，此汇率适用招行信用卡还款和兑换外汇外钞，交易以实际兑换汇率为准。点击货币名称可以单独查询此外币币种的今日实时牌价并可以输入数额换算兑换汇率。
      </template>
    </Summary>
    <!--十大银行汇率，排除当前选中的银行 -->
    <bank-list filterOption="CMBCHINA"></bank-list>
  </div>
</template>
...
export default {
  ...
  head() {
    return {
      title: '今日最新招商银行汇率 招行外汇牌价查询',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: '提供中国招商银行当日美元及人民币等10种主要货币的即时外汇牌价,招行汇率查询服务和汇率计算工具。'
        },
        {
          hid: 'keywords',
          name: 'keywords',
          content: '招商银行汇率,招商银行外汇牌价,招商银行外汇汇率,招商银行,招行'
        }
      ]
    }
  },
  async asyncData({ $axios }) {
    ...
    //包含的货币
    let tableData = []
    const data = await $axios.$get('/105-32?bankCode=CMBCHINA', { timeout: 3000 })
