<template>
  <div class="content">
    ...
    <Summary title="中信银行外汇牌价表使用说明">
      <template slot="text">
        提供中国中信银行当日美元、欧元、日元等主要货币兑人民币的外汇牌价表，每100单位外币兑人民币。本表中的<strong>现汇买入价:</strong>指银行买入外汇的汇率，<strong>现钞买入价:</strong>指银行买入外币现钞的汇率，<strong>现汇卖出价:</strong>指银行卖出外汇的汇率，<strong>现钞卖出价:</strong>指银行卖出外钞的汇率。外汇牌价数据引用来自中信银行，此汇率适用中信信用卡还款和兑换外汇外钞，交易以实际兑换汇率为准。点击货币种类可以查询此币种外币的今日实时汇率并可以输入数额计算兑换汇率。
      </template>
    </Summary>
    <!--十大银行汇率，排除当前选中的银行 -->
    <bank-list filterOption="ECITIC"></bank-list>
  </div>
</template>
