...
export default {
  ...
  head() {
    return {
      title: '今日最新兴业银行汇率 兴业银行外汇牌价查询',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: '提供中国兴业银行当日美元及人民币等主要货币的即时外汇牌价,兴业汇率查询服务和汇率兑换计算工具。'
        },
        {
          hid: 'keywords',
          name: 'keywords',
          content: '兴业银行汇率,兴业银行外汇牌价,兴业银行外汇汇率,兴业银行,兴业'
        }
      ]
    }
  },
  async asyncData({ $axios }) {
    ...
    //包含的货币
    let tableData = []
    const data = await $axios.$get('/105-32?bankCode=CIB', { timeout: 3000 })
