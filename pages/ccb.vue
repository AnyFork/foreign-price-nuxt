<template>
  <div class="content">
    <!--顶部title -->
    <title-item title="今日建设银行外汇牌价汇率表" :subtitle="'建行外汇牌价更新时间：' + refresh"></title-item>
    <a-divider />
    <!--广告位 -->
    ...
    <a-divider />
    <!--文本摘要 -->
    <Summary title="建设银行外汇牌价表使用说明">
      <template slot="text">
        提供中国建设银行当日美元、欧元、日元等主要货币兑人民币的即时外汇牌价表，单位为人民币兑100外币。本表中的<strong>现汇买入价:</strong>指银行买入外汇的汇率，<strong>现钞买入价:</strong>指银行买入外币现钞的汇率，<strong>现汇卖出价:</strong>指银行卖出外汇的汇率，<strong>现钞卖出价:</strong>指银行卖出外钞的汇率。外汇牌价数据随建行营业网点实时更新，此汇率适用建行信用卡还款和兑换外汇外钞，交易以实际兑换汇率为准。点击货币种类可以单独查询此外币币种的今日实时牌价并可以输入数额换算兑换汇率。
      </template>
    </Summary>
    <!--十大银行汇率，排除当前选中的银行 -->
    <bank-list filterOption="CCB"></bank-list>
  </div>
</template>
...
export default {
  ...
  async asyncData({ $axios }) {
    ...
    //包含的货币
    let tableData = []
    const data = await $axios.$get('/105-32?bankCode=CCB', { timeout: 3000 })
